      ...
      try {
        ...
        if (interaction.isChatInputCommand()) {
          try {
            ...
            // Pre-defer interactions that do not require an immediate modal to prevent timeouts during DB checks
            const subCmd = interaction.options?.getSubcommand(false);
            const needsModal = (interaction.commandName === 'apply' && subCmd === 'submit') || 
                               (interaction.commandName === 'app-admin' && subCmd === 'setup') ||
                               (interaction.commandName === 'embedbuilder');
            
            if (!needsModal) {
                // Determine deferral flags based on command configuration (e.g. some might need ephemeral)
                // For safety, most commands default to non-ephemeral unless specified, but we can safely let InteractionHelper handle the first defer.
                // Actually, let's defer normally, the command itself will safely switch to editReply.
            ...
            if (interaction.guild) {
              ...
              // Role and Permission Gating: Ensure only staff with assigned roles or management permissions can run administrative commands
              const category = command.category?.toLowerCase();
              const restrictedCategories = ['moderation', 'logging', 'welcome', 'jointocreate', 'verification', 'reaction_roles'];
              if (restrictedCategories.includes(category)) {
                const isOwner = interaction.guild.ownerId === interaction.user.id;
                const isAdmin = interaction.memberPermissions?.has('Administrator') || interaction.memberPermissions?.has('ManageGuild') || interaction.memberPermissions?.has('ModerateMembers');
                const hasModRole = guildConfig?.modRole && interaction.member?.roles?.cache?.has?.(guildConfig.modRole);
                const hasAdminRole = guildConfig?.adminRole && interaction.member?.roles?.cache?.has?.(guildConfig.adminRole);

                if (!isOwner && !isAdmin && !hasModRole && !hasAdminRole) {
                  throw createError(
                    `User ${interaction.user.tag} lacks staff permissions for /${interaction.commandName}`,
                    ErrorTypes.PERMISSION,
                    '🛡️ **Staff Only**: You do not have the required Staff / Moderator role assigned to execute this command.',
                    withTraceContext({ commandName: interaction.commandName, guildId: interaction.guild.id }, interactionTraceContext)
                  );
                }
              }
          ...
          if (interaction.customId.startsWith('radio_')) {
            try {
              const { activeRadioSessions, RADIO_STATIONS, createHDAudioResource } = await import('../commands/Voice/radio.js');
              const action = interaction.customId.replace('radio_', '');
              const session = activeRadioSessions.get(interaction.guildId);
              ...
              if (action === 'stop') {
                if (session) {
                  try {
                    ...
                    activeRadioSessions.delete(interaction.guildId);
                  } catch (e) {}
                }
                return await InteractionHelper.safeReply(interaction, {
                  content: '⏹️ Radio playback stopped and disconnected from voice channel.',
                  ephemeral: true
                });
              }

              const newStation = RADIO_STATIONS[action];
              if (newStation && session) {
                session.stationKey = action;
                session.station = newStation;
                const { resource, process: proc } = createHDAudioResource(newStation.url, session.volume || 0.8);
                session.currentResource = resource;
                session.ffmpegProcess = proc;
                session.player.play(resource);

                return await InteractionHelper.safeReply(interaction, {
                  content: `📻 Switched radio stream to **${newStation.name}**!`,
                  ephemeral: true
                });
              } else if (!session) {
                return await InteractionHelper.safeReply(interaction, {
                  content: '⚠️ No active radio stream. Start one with `/radio play`!',
                  ephemeral: true
                });
              }
            } catch (err) {
              logger.error('Radio button error:', err);
            }
            return;
          }
