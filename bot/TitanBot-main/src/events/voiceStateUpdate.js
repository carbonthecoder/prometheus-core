...
const voiceJoinTimes = new Map(); // Tracks when users join voice channels
    ...
    async execute(oldState, newState, client) {
        ...
        try {
            ...
            if (!oldState.channel && newState.channel) {
                // --- VOICE ECONOMY: Record Join Time ---
            ...
            if (oldState.channel && !newState.channel) {
                // --- VOICE ECONOMY: Calculate and Reward ---
                if (voiceJoinTimes.has(userId)) {
                    ...
                    if (minutesSpent > 0) {
                        ...
                        try {
                            await EconomyService.addMoney(client, guildId, userId, coinsEarned, 'Voice Activity Reward');
                            logger.info(`[VOICE ECONOMY] Awarded ${coinsEarned} coins to ${userId} for ${minutesSpent} minutes of voice chat.`);
                        } catch (err) {
                            logger.error(`[VOICE ECONOMY] Failed to reward user:`, err);
                        }
                    }
                }
                // -------------------------------------------
